...
=\mQ\mLambda\mQ^\mathsf{T}=\mSigma$, just as for the Cholesky factor
...
- [Transport maps and normalizing flows](#transport-maps-and-normalizing-flows)
...
$$
\vX-\vmu
=\sqrt{2+\sqrt2}\,\vq_1\Phi^{-1}(U_1)
+\sqrt{2-\sqrt2}\,\vq_2\Phi^{-1}(U_2)
$$
...
## Transport maps and normalizing flows {.tree-marker-slide tree-marker="random-generation"}

The quantile transform is a [one-dimensional transport]{.alert}. In $d$
dimensions, start with an easy reference density $\varrho_0$ and an invertible,
...
$$
\varrho_X(\vx)
=\varrho_0\!\left(T^{-1}(\vx)\right)
\left|\det \nabla T^{-1}(\vx)\right|
$$

A [normalizing flow]{.alert} composes manageable maps
$T=T_m\circ\cdots\circ T_1$, often tuning their parameters to fit a target

::: {.key-point}
A transport [moves every sample and keeps equal weights]{.alert}; importance
sampling keeps proposal samples and assigns unequal weights
:::
...
One layer turns independent Gaussian coordinates into a curved, dependent
target. If no useful invertible map is available, [acceptance–rejection]{.alert}
offers another route from an easy proposal to the target
