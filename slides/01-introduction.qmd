...
format:
  revealjs:
    theme:
      - simple
      - ../classlib/classlib/quarto/slides/hickernell-slides.scss
      - 01-introduction-tree.scss
...
\text{service time}
\end{cases} \\
& = f(\boldsymbol X), \text{ where} \\
\boldsymbol X & = \text{multivariate random variable that is }\alerttext{easy to sample}
\end{align*}
...
for repository_root in (Path.cwd(), *Path.cwd().parents):
    ...
    if (tree_components / "trees" / "__init__.py").is_file():
        sys.path.insert(0, str(tree_components.resolve()))
        break
else:
